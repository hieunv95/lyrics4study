    ...
    <div class="top-grids">
        ...
        <div class="col-md-4 resent-grid recommended-grid slider-top-grids">
            ...
            <div class="resent-grid-info recommended-grid-info">
                ...
                <ul>
                    <li>
                        <p class="author author-info"><a href="#" class="author">{{ $lyric->user->name }}</a></p>
                    </li>
                    <li class="right-list">
        ...
        <div class="recommended-grids">
            ...
            <div id="top" class="callbacks_container">
                <ul class="rslides" id="slider3">
                    @for ($i = $j = 0; $i < $slideNumber; $i++)
                    <li>
                        <div class="animated-grids">
                            @foreach ($newLyrics as $lyricId => $lyric)
                            @continue(in_array($lyricId, $firstSlideLyrics) && $i > 0)
                            <div class="col-md-3 resent-grid recommended-grid slider-first">
                                <div class="resent-grid-img recommended-grid-img">
                                    <a href="{{ route('show', ['id' => $lyric->id,
                                        'title' => str_slug($lyric->title,'-'),
                                        'artist' => str_slug($lyric->artist,'-')]) }}">
                                        <img src="https://i3.ytimg.com/vi/{{ $lyric->link_id }}/mqdefault.jpg"
                                        alt="{{ $lyric->title }}-{{ $lyric->artist }}" />
                                    ...
                                    <h5>
                                        <ul>
                                            <li><a href="{{ route('show',
                                                    ...
                                                    'title' => str_slug($lyric->title,'-'),
                                                    'artist' => str_slug($lyric->artist,'-')]) }}"
                                                    class="title">{{ $lyric->title }}
                                                </a>
                                            </li>
                                            <li>
                                                <a href="{{ action("Web\HomeController@search", [
                                                    'q' => $lyric->artist,
                                                    'search_param' => 'artist',
                                                ]) }}" class="artist artist-info" >
                                                    {{ $lyric->artist }}
                                                </a>
                                            </li>
                                        </ul>
